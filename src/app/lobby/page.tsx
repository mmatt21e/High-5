...
export const metadata: Metadata = { title: "Lobby | Five-O Poker", robots: { index: false } };

export default async function LobbyPage() {
  const session = await auth();
  if (!session?.user?.id) redirect("/login?callbackUrl=/lobby");
  ...
  return (
    <main className="app-screen flex flex-1 flex-col">
      <header className="app-header flex items-center justify-between">
        <div>
          <h1 className="app-title text-2xl font-black">Game lobby</h1>
